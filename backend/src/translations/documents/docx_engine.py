...
class DocxTranslationEngine:
    ...
    def _parse(self) -> DocumentTree:
        root = Section(title="(document)", level=0)
        segments: list[Segment] = []
        # Stack of (level, section); new headings pop back to their parent.
        ...
        def add_segment(
            paragraph: Paragraph, *, in_table: bool, is_heading: bool
        ) -> None:
            kind = classify_text(
            ...
            if isinstance(item, Paragraph):
                ...
                if level is not None and item.text.strip():
                    while stack[-1].level >= level:
                        stack.pop()
                    section = Section(title=item.text.strip(), level=level)
                ...
                self._parse_table(item, add_segment)

        return DocumentTree(root=root, segments=segments)

    def _parse_table(self, table: Table, add_segment) -> None:
        seen_cells: set[int] = set()  # merged cells repeat the same <w:tc>
        for row in table.rows:
            for cell in row.cells:
                ...
                for paragraph in cell.paragraphs:
                    add_segment(paragraph, in_table=True, is_heading=False)
                for nested in cell.tables:
                    self._parse_table(nested, add_segment)
    ...
    def apply(self) -> int:
        ...
        for seg in self.tree.segments:
            if seg.translation is None or seg.translation == seg.text:
                continue
            texts = seg.paragraph._p.findall(f".//{_W_NS}t")
            if not texts:
                logger.warning(
