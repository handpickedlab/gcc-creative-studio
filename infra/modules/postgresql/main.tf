...
resource "google_sql_database_instance" "default" {
  ...
  settings {
    # Enterprise custom-1 (1 vCPU / 3.75 GiB) instead of the upstream
    # Enterprise Plus db-perf-optimized-N-2 (2 vCPU / 16 GiB). Cloud SQL
    # cannot scale to zero, so this is the 24/7 hosting bill.
    tier              = var.db_tier
